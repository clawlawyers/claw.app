...
const generateInvoicePDF = async (userId, orderId) => {
  ...
  // Pipe PDF into a buffer instead of response for later usage
  doc.on('data', chunk => pdfBuffer.push(chunk));
  doc.on('end', () => {});

  // Invoice Header
  doc.fontSize(20).text('Claw Legaltech', 50, 50);
  doc.fontSize(10).text('www.clawlaw.in', 50, 70);

  // User Details
  doc.moveDown().fontSize(14).text('User Details');
  doc.fontSize(10)
      .text(`Name / Mobile No.: ${user.firstName} ${user.lastName} / ${user.phoneNumber}`, 50, 120)
      .text(`Invoice Date: ${new Date().toLocaleDateString()}`, 50, 140);

  // Plan Details
  doc.moveDown().fontSize(14).text('Plan Details');
  doc.fontSize(10)
      .text(`Plan Name: ${order.plan}`, 50, 180)
      .text(`Plan Start Date: ${order.createdAt.toLocaleDateString()}`, 50, 200)
      .text(`Plan End Date: ${order.billingCycle === 'MONTHLY' ? new Date(order.createdAt).setMonth(order.createdAt.getMonth() + 1) : new Date(order.createdAt).setFullYear(order.createdAt.getFullYear() + 1)}`, 50, 220)
      .text(`Session: ${order.session}`, 50, 240)
      .text(`Rate: ₹${order.request || '0'} /-`, 400, 240);

  // Discount and Total
  const discountPercentage = order.discountPercentage || 0;
  const discountAmount = (discountPercentage / 100) * order.request;
  const totalAmount = order.request - discountAmount;

  doc.text(`Discount Percentage: ${discountPercentage}%`, 50, 270)
     .text(`Coupon Code: ${order.couponCode || 'N/A'}`, 50, 290)
     .text(`₹ ${discountAmount.toFixed(2)} /-`, 400, 270);

  // Total Amount
  doc.fontSize(12).text(`Total: ₹${totalAmount.toFixed(2)} /-`, 400, 320);
