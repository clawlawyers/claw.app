...
// Service to generate the invoice PDF
async function generateInvoicePDF(userId, planName) {
  try {
    // Fetch user and their plan details from the database
    console.log('Fetching user plan details for userId:', userId, 'and planName:', planName);
    const userPlan = await prisma.newUserPlan.findUnique({
      where: {
        userId_planName: { userId: String(userId), planName: String(planName) }, // Ensure userId and planName are strings
      },
      include: {
    ...
    doc.fontSize(10).text(`Name / Mobile No.: ${userName} / ${phoneNumber}`, 50, 120);
    ...
    doc.text(`Plan Name: ${planName}`, 50, 200);  // Use the planName parameter here
    ...
    doc.text(`Rate: ₹${Paidprice.toFixed(2)} /-`, 400, 215);
    ...
    doc.moveDown().fillColor('#FFFFFF').rect(50, 275, 500, 20).fill('#004D40');
    doc.fillColor('#FFFFFF').fontSize(12).text('Total', 60, 280);
    doc.fillColor('#333333').fontSize(12).text(`₹${Paidprice.toFixed(2)} /-`, 400, 280);

    // Referral Code Section (if applicable)
    if (referralCode) {
    ...
    // End the document and return the PDF buffer
    doc.end();
    return Buffer.concat(pdfBuffer);

  } catch (error) {
    console.error('Error in generateInvoicePDF service:', error);  // Log the error
    throw new AppError('Error while generating invoice', 500);  // Throw a custom error
  }
}
