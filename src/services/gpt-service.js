...
// Service function to generate the invoice PDF
async function generateInvoicePDF(userId, planName) {
  try {
    // Fetch user and their plan details from the database
    const userPlan = await prisma.newUserPlan.findUnique({
      where: {
        userId_planName: { userId, planName },
      },
      include: {
        user: true,  // Include user details
        plan: true,  // Include plan details
        referralCode: true  // Include referral code details if available
      },
    });
    ...
    // Extract plan and user details
    const { user, plan, referralCode } = userPlan;
    const { firstName, lastName, phoneNumber } = user;
    const { name } = plan;  // Using `name` directly instead of renaming to `planName`
    const { isCouponCode, createdAt, expiresAt, Paidprice } = userPlan;
    ...
    // Header
    doc.fontSize(20).text('Claw Legaltech', 50, 50);
    doc.fontSize(10).text('www.clawlaw.in', 50, 70, { align: 'right' });

    // User Details
    doc.moveDown().fontSize(12).text('User Details');
    doc.fontSize(10)
      .text(`Name / Mobile No.: ${firstName} ${lastName} / ${phoneNumber}`, 50, 100)
      .text(`Invoice Date: ${new Date().toLocaleDateString()}`, 50, 115);

    // Plan Details Section
    doc.moveDown().fontSize(12).text('Plan Details');
    doc.fontSize(10)
      .text(`Plan Name: ${name}`, 50, 150)  // Using `name` instead of `planName`
      .text(`Plan Start Date: ${createdAt.toLocaleDateString()}`, 50, 165)
      .text(`Plan End Date: ${expiresAt ? new Date(expiresAt).toLocaleDateString() : 'N/A'}`, 50, 180)
      .text(`Rate: ₹${Paidprice.toFixed(2)} /-`, 400, 150);

    // Coupon Code (if applicable)
    if (isCouponCode) {
      doc.text(`Coupon Code: ${isCouponCode}`, 50, 210);
    }

    // Total Amount
    doc.fontSize(12).text(`Total: ₹${Paidprice.toFixed(2)} /-`, 400, 240);

    // Referral Code
    if (referralCode) {
      doc.moveDown().fontSize(10).text(`Referral Code: ${referralCode.referralCode}`, 50, 260);
    }

    // Footer Note
    doc.moveDown().fontSize(8)
      .text('NOTE: This Invoice Is Generated By Claw Legaltech Website.')
      .text('If You Have Any Queries, Kindly Contact Administrator.');
    ...
    throw new AppError('Error while generating invoice', 500);
  }
}
