...
async function verifyClientMiddleware(req, res, next) {
  try {
    ...
    if (data.verified) {
      await clientVerifySchema.validateAsync({
...
async function validateLawyerRegisterRequest(req, res, next) {
  try {
    ...
    req.body.phoneNumber = req.body.phoneNumber.toString();
    if (!req.file?.buffer)
      throw new AppError("Missing identification attachment");
...
function validateSignUpRequest(req, res, next) {
  if (!req.body.username) {
    ...
    ErrorResponse.error = new AppError(
      ["username not found in the incoming request"],
...
async function validateAuthRequest(req, res, next) {
  if (!req.headers["authorization"]) {
    ...
    ErrorResponse.error = new AppError(
      ["token not found in the incoming request"],
      StatusCodes.BAD_REQUEST
    );
    return res.status(StatusCodes.BAD_REQUEST).json(ErrorResponse);
  }
...
function validatePostRequest(req, res, next) {
  if (!req.body.description || req.body.description === "") {
    ...
    ErrorResponse.error = new AppError(
      ["Description is not found in the incoming message"],
      StatusCodes.BAD_REQUEST
    );
    return res.status(StatusCodes.BAD_REQUEST).json(ErrorResponse);
  }
  if (!req.body.price_range || req.body.price_range === "") {
    ...
    ErrorResponse.error = new AppError(
      ["Price Range is not found in the incoming message"],
      StatusCodes.BAD_REQUEST
    );
    return res.status(StatusCodes.BAD_REQUEST).json(ErrorResponse);
  }
...
function validatePostUpdateRequest(req, res, next) {
  if (!req.body.id && req.body.id === "" && req.query.id === "") {
    ...
    ErrorResponse.error = new AppError(
      "Post id not found in the incoming message",
      StatusCodes.BAD_REQUEST
    );
    return res.status(StatusCodes.BAD_REQUEST).json(ErrorResponse);
  }
